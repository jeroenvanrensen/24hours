...
<li class="relative" x-data="{ {{ $modalName }}: false }">
    <a href="{{ $url }}" class="block group border border-gray-200 rounded-lg focus:outline-none overflow-hidden" {{ $newTab ? 'target="_blank"' : '' }}>
        ...
        <div class="px-6 py-4">
            ...
            <!-- Meta -->
            <span class="flex-1 inline-flex items-center bg-gray-100 py-px px-2 rounded-full text-sm font-semibold">
                {{ $meta }}
            </span>
        </div>
    </a>
    ...
    <!-- Delete button -->
    <button @click="{{ $modalName }} = true" class="absolute top-2 right-2 h-5 w-5 bg-gray-100 text-gray-600 rounded-full flex items-center justify-center hover:bg-gray-300 focus:outline-none focus:bg-gray-400">
