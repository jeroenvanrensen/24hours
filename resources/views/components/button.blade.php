...
<{{ $link ? 'a' : 'button' }} {{ $attributes->merge([
    ...
    <svg wire:loading wire:target="{{ $loading }}" class="w-5 h-5 mr-2 -mt-1 -ml-1 text-white animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
...
<{{ $link ? 'a' : 'button' }} {{ $attributes->merge([
    'class' => 'py-2 px-4 md:px-6 bg-' . $color . '-800 rounded text-white transition hover:bg-' . $color . '-900 focus:bg-' . $color . '-900 focus:outline-none focus:ring-2 ring-' . $color . '-400',
    'wire:loading.class' => 'opacity-50 pointer-events-none',
    'wire:target' => $loading
]) }}>
    <svg wire:loading wire:target="{{ $loading }}" class="w-5 h-5 mr-2 -mt-1 -ml-1 text-white animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
