<div x-data="{ showDropup: false, showModal: false }" x-init="$wire.on('hideModal', () => {showModal = false});">
    ...
    <!-- Create note dropup -->
    <button x-show="showDropup" wire:click="createNote" class="fixed bottom-32 right-12 p-4 bg-gray-400 hover:bg-gray-500 rounded-full transition transform focus:outline-none focus:bg-gray-600 text-white" x-transition:enter="delay-150 ease-out duration-300" x-transition:enter-start="opacity-0 scale-50" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="delay-150 ease-out duration-300" x-transition:leave-start="opaicty-100 scale-100" x-transition:leave-end="opacity-0 scale-50">
        <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <!-- Add item button -->
    <button @click="showDropup = !showDropup" @click.away="showDropup = false" class="fixed bottom-12 right-12 p-3 bg-blue-800 hover:bg-blue-900 rounded-full transition duration-300 focus:outline-none focus:bg-blue-900 text-white" :class="{ 'transform rotate-45': showDropup }">
